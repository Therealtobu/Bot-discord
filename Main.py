...
async def on_message(message):  
    ...
    # Phát hiện link bypass (giả sử bypassed là kết quả bạn lấy từ API)
    if "http" in content:  
        bypassed = "Đây là nội dung bypass của link, giả sử rất dài..." * 500  # ví dụ giả  
        MAX_LENGTH = 3900  

        if bypassed:  
            bypass_str = str(bypassed)  
            if len(bypass_str) > MAX_LENGTH:  
                bypass_str = bypass_str[:MAX_LENGTH] + "..."  

            await message.reply(f"🔓 Link đã bypass:\n{bypass_str}")  
            return  
